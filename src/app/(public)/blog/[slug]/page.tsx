  ...
  const flushList = () => {
    if (currentList.length > 0) {
      elements.push(
        <ul
          key={key++}
          className="list-disc list-outside ml-5 text-base sm:text-lg text-white/90 space-y-2 mb-6"
        >
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith("> ")) {
      ...
      elements.push(
        <blockquote
          key={key++}
          className="my-8 border-l-2 border-white/20 pl-5 text-base sm:text-lg italic text-white/70 leading-relaxed"
        ...
        <article className="mx-auto max-w-3xl px-4 sm:px-6">
          ...
          <header className="mb-10">
            <div className="flex items-center gap-3 mb-4">
              <span className="text-base font-medium tracking-widest text-white uppercase">
                {article.category}
              </span>
          ...
          {article.tags.length > 0 && (
            <div className="mt-12 flex flex-wrap gap-2">
              {article.tags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full border border-white/[0.08] bg-white/[0.02] px-3 py-1 text-base text-white/60 tracking-wide"
                >
                  {tag}
                </span>
              ))}
          ...
          {relatedArticles.length > 0 && (
            <div className="mt-14 border-t border-white/[0.06] pt-10">
              <p className="text-base font-semibold tracking-widest text-white/70 uppercase mb-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {relatedArticles.map((related) => (
                  ...
                  >
                    ...
                    <p className="text-base font-medium text-white mt-2 leading-snug group-hover:text-white/90 transition-colors">
                      {related.title}
                    </p>
                    <span className="inline-flex items-center gap-1 text-base text-white/60 mt-3 group-hover:text-white/80 transition-colors">
                      Lire
                      <ArrowRight
                        size={10}
            ...
            <div className="flex flex-col sm:flex-row gap-6 sm:gap-8">
              {prevArticle ? (
                ...
                >
                  ...
                  <p className="text-base font-medium text-white/80 mt-1.5 group-hover:text-white transition-colors">
                    {prevArticle.title}
                  </p>
                </Link>
              ) : (
