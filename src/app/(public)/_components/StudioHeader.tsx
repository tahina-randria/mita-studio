      ...
      >
        ...
        <nav aria-label="Navigation principale" className="flex items-center justify-between w-[calc(100%-2rem)] max-w-6xl mx-auto mt-[max(0.5rem,env(safe-area-inset-top))] md:mt-[max(1rem,env(safe-area-inset-top))] rounded-2xl bg-[#0a0f25] md:bg-white/[0.04] md:backdrop-blur-2xl md:backdrop-saturate-150 border border-white/[0.06] px-4 sm:px-6 py-2.5">
            ...
            {navLinks.map((link) =>
              !link.href.startsWith('#') ? (
                <Link
                  key={link.id}
                  href={link.href}
                  className="px-3.5 py-1.5 text-base text-white hover:text-white rounded-lg hover:bg-white/[0.04] transition-colors"
                >
                  {link.label}
                </Link>
              ) : (
          ...
          <div className="flex items-center gap-2">
            <Link
              href="/contact"
              className="inline-flex items-center rounded-lg bg-white px-4 py-2 text-base font-semibold text-[#0a0a0a] hover:bg-white/90 active:scale-[0.98] transition-all shrink-0"
            >
              Nous contacter
            </Link>
            {/* Mobile hamburger */}
